...
<main>
    ...
    <section class="menu">
        <nav class="menuNav">
            <a href="<?= '/ucet/' . $uzivatel->id . '/clanky' ?>">Články uživatele</a>
            <a href="<?= '/ucet/' . $uzivatel->id . '/komentare' ?>">Komentáře uživatele</a>
            <a class="active" href="<?= '/ucet/' . $uzivatel->id . '/oblibene' ?>">Oblíbené články uživatele</a>
        </nav>
    </section>
    ...
    <section class="panel">
        <div class="subPanel">
            <?php
            foreach ($hodnoceniClanku as $hodnoceni) {
                if ($hodnoceni->hodnoceni == 1) {

                    $vybranyClanek = array_filter($clanky, function ($clanek) use ($hodnoceni) {
                        return $clanek->id === $hodnoceni->idClanek;
                    });
                    $vybranyClanek = reset($vybranyClanek);

                    if ($vybranyClanek && $vybranyClanek->idStav == 4) {

                        $vybraneObrazky = array_filter($obrazky, function ($obrazek) use ($vybranyClanek) {
                            return $obrazek->idClanek === $vybranyClanek->id;
                        });
                        $vybranyObrazek = reset($vybraneObrazky);
                        $obrazekSrc = $vybranyObrazek ? $vybranyObrazek->cesta : '/assets/img/default.jpg';
            ?>
                        <article class="article">
                            <div class="articleVedle">
                                <a class="obrazek flex1" href="/clanek/<?= $vybranyClanek->id ?>"><img src="<?= $obrazekSrc ?>" alt="<?= $vybranyObrazek ? htmlspecialchars($vybranyObrazek->alt) : 'no image avaible' ?>"></a>
                                <div class="textClanek flex2">
                                    <div>

                                        <div class="clanek">
                                            <a href="/clanek/<?= $vybranyClanek->id ?>">
                                                <h2><?= preg_replace("/(?<=\s)\b([\w]{1,2})\b\s/", "$1&nbsp;", htmlspecialchars($vybranyClanek->nadpis))  ?></h2>
                                            </a>
                                            <p class="upoutavka"><?= preg_replace("/(?<=\s)\b([\w]{1,2})\b\s/", "$1&nbsp;", htmlspecialchars($vybranyClanek->upoutavka))  ?></p>
                                        </div>
                                        <div class="casKat">
                                            <h6 class="cas">
                                                <?php
                                                ...
                                                $casString = $vybranyClanek->createdAt->format("Y-m-d H:i:s");
                                            ...
                                            <h6 class="kategorie"><a href="/filtr/kategorie/<?= $kategorie[$vybranyClanek->idKategorie - 1]->id ?>"><?= htmlspecialchars($kategorie[$vybranyClanek->idKategorie - 1]->nazev) ?></a></h6>
                                        </div>
                                    </div>
                                    <?php
                                    if (isset($_SESSION["uzivatel_id"]) && $uzivatel->id == $_SESSION["uzivatel_id"]) {

                                        $vybraneHodnoceniClanku = array_filter($hodnoceniClanku, function ($hodnoceni) use ($vybranyClanek) {
                                            return $hodnoceni->idClanek === $vybranyClanek->id;
                                        });
                                    ...
                                    ?>
                                            <form method="post">
                                                <input type="hidden" name="idClanek" value="<?= $vybranyClanek->id ?>">
